"""
Send a FASTA to the vsearch proxy server and write annotated results as Parquet.

Sequences are chunked into batches and POSTed to /search/batch. The best match
for each query (as selected by the proxy's pickBestMatch) is written as one row
in the output Parquet. All 23 FASTA header fields from the reference database
are included alongside alignment statistics.

Usage:
    python3 analysis/annotate_sequences.py <fasta_file> [options]

Examples:
    python3 analysis/annotate_sequences.py tests/input/musca.fasta
    python3 analysis/annotate_sequences.py tests/input/musca.fasta \\
        --output output/musca_annotated.parquet --batch-size 50
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Annotate sequences via vsearch proxy and write Parquet"
    )
    parser.add_argument("fasta_file",   help="Input FASTA file")
    parser.add_argument("--output",     help="Output Parquet path (default: output/<stem>_annotated.parquet)")
    ...
    output_path = Path(args.output) if args.output \
        else Path("output") / f"{fasta_path.stem}_annotated.parquet"
    ...
    con = duckdb.connect()
    con.execute(f"COPY (SELECT * FROM read_json('{tmp_path}')) TO '{output_path}' (FORMAT PARQUET)")
